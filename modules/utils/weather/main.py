...
def get_openweather_api_key():
    """Đọc API key từ txa.json"""
    try:
        import json
        with open('txa.json', 'r', encoding='utf-8') as f:
            data = json.load(f)
            bot_data = data.get("data", [{}])[0]
            api_key = bot_data.get("openweather_api_key")
            return api_key
    except Exception as e:
        print(f"[ERROR] Lỗi đọc OpenWeather API key: {e}")
        return None
...
def fetch_weather_info(area):
    ...
    if not api_key:
        return "❌ OpenWeather API key không được cấu hình. Vui lòng thêm 'openweather_api_key' vào txa.json"
    ...
    if 'main' not in data or 'weather' not in data:
        return "❌ Không thể lấy thông tin thời tiết từ OpenWeather API."

    current_temp = data['main']['temp']
    min_temp = data['main']['temp_min']
    max_temp = data['main']['temp_max']
    humidity = data['main']['humidity']
    description = data['weather'][0]['description'] if data['weather'] else "Không xác định"
    weather_id = data['weather'][0]['id'] if data['weather'] else 0
    
    # Convert OpenWeather code to our format if needed
    weather_info = create_weather_message_openweather(area, description, min_temp, max_temp, current_temp, humidity, weather_id)
    return weather_info
def create_weather_message_openweather(area, description, min_temp, max_temp, current_temp, humidity, weather_id):
    """Tạo tin nhắn thời tiết từ dữ liệu OpenWeather API"""
    # Emoji cho các loại thời tiết
    weather_emojis = {
        200: "⛈️ Dông bão với mưa nhẹ",
        201: "⛈️ Dông bão với mưa",
        202: "⛈️ Dông bão với mưa lớn",
        210: "🌩️ Dông bão nhẹ",
        211: "🌩️ Dông bão",
        212: "🌩️ Dông bão nặng",
        221: "🌩️ Dông bão rải rác",
        230: "⛈️ Dông bão với mưa phùn nhẹ",
        231: "⛈️ Dông bão với mưa phùn",
        232: "⛈️ Dông bão với mưa phùn lớn",
        300: "🌦️ Mưa phùn nhẹ",
        301: "🌦️ Mưa phùn",
        302: "🌧️ Mưa phùn lớn",
        310: "🌧️ Mưa phùn nhẹ",
        ...
        801: "🌤️ Mây ít",
        802: "⛅ Mây rải rác",
        803: "🌥️ Mây chiếm nhiều",
        804: "☁️ Mây nhiều"
    }
    
    emoji = weather_emojis.get(weather_id, f"🌍 {description}".title())
    ...
    message = (
        ...
        f"🌡 Nhiệt độ thấp nhất: {min_temp}°C\n"
        f"🌡 Nhiệt độ cao nhất: {max_temp}°C\n"
        f"💧 Độ ẩm: {humidity}%\n"
    )
    return message
