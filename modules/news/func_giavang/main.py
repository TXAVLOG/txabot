...
def generate_menu_image(bot, author_id, thread_id, thread_type):
    ...
    try:
        ...
        text_lines = [
            ...
            f"{bot.prefix}giavang on/off: 🚀 Bật/Tắt tính năng ",
            "😁 Bot Sẵn Sàng Phục 🖤",
...
def get_gold_price_vn():
    ...
    if data["success"]:
        ...
        result = [f"🚦 Cập nhật lúc: {latest_date}"]
        ...
        for province, gold_list in provinces.items():
            ...
            for gold_type, buy_price, sell_price in gold_list:
                ...
                result.append(f"   🛒 Mua vào: {buy_price}")
                result.append(f"   💰 Bán ra: {sell_price}")
...
def handle_giavang_on(bot, thread_id):
    ...
    return f"🚦Lệnh {bot.prefix}giavang đã được Bật 🚀 trong nhóm này ✅"
...
def handle_giavang_off(bot, thread_id):
    ...
    if "giavang" in settings and thread_id in settings["giavang"]:
        ...
        write_settings(bot.uid, settings)
        return f"🚦Lệnh {bot.prefix}giavang đã Tắt ⭕️ trong nhóm này ✅"
    return "🚦Nhóm chưa có thông tin cấu hình giavang để ⭕️ Tắt 🤗"
...
def handle_gia_vang_command(bot, message_object, author_id, thread_id, thread_type, command):
    def gia_vang():
        ...
        try:
            ...
            if len(parts) == 1:
                response = (
                    "🎉 Chào mừng đến với menu Gia Vàng! ⚙️\n"
                    f"   ➜ {bot.prefix}giavang [vn]: xem giá vàng Việt Nam 💰\n"
                )
            else:
...
txa = {
    "name": "pro_giavang",
    "desc": "Xem giá vàng cập nhật theo thời gian thực. Hỗ trợ xem giá vàng các loại và gửi vào nhóm. Admin có thể bật/tắt tính năng.",
    "author": "TXA",
    "command": ['gia_vang']
}
...
def txa_command(bot, message_object, thread_id, thread_type, author_id, message_text):
    ...
    dispatch_map = {
        'gia_vang': handle_gia_vang_command
    }
