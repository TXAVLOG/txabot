...
def handle_hiden_commands(message, thread_id, thread_type, author_id, bot, message_object):
    ...
    command_names = "".join([
        ...
        f"➜ 💪 Kích toàn bộ member ({bot.prefix}kickall)\n"
        f"➜ 🍭 join link nhóm ({bot.prefix}join)\n"
        f"➜ 🆘 Giải tán nhóm: ({bot.prefix}disbox)\n"
        f"➜ 🚨 Spam call ({bot.prefix}call [số lượng] @tag)\n"
        f"➜ 🧨 spam link nhóm ({bot.prefix}spam)\n"
        f" ➜ ☕ Xem source tin nhắn: ({bot.prefix}src)\n"
        f"➜ 👋 Rời nhóm ({bot.prefix}leave)\n"
        f"➜ 😪 Kết Bạn Trong Gr({bot.prefix}kbgr)\n"
        f"➜ 🇻🇳 Block Bạn Bè ({bot.prefix}block)\n"
        f"➜ 🐼 Mở Block ({bot.prefix}unblock)\n"
        f"➜ 😋 Xóa Kb({bot.prefix}xkb)\n"
        f"➜ 😆 Kết Bạn ({bot.prefix}kb)\n"
