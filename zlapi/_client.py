...
class ZaloAPI(object):
	...
	def onLoggingIn(self, phone=None):
		...
		"""
		logger.debug("API TBOT".format(phone))
	...
	def onLoggedIn(self, phone=None):
		...
		Args:
			phone: The phone number of the client
		"""
		logger.login("TBOT ĐÃ ĐĂNG NHẬP VÀO SỐ {} NÀY🦍.".format(phone))
	
	def onListening(self):
		"""Called when the client is listening."""
		logger.debug("BẠN ĐANG DÙNG API TBOT 1.0.3😁")
	...
	def updateAutoDeleteChat(self, ttl, threadId, isGroup=True):
		"""Update auto delete chat (TTL) for a group or user conversation.

		Args:
			ttl (int): The time to live of the chat in milliseconds.
			           Valid values: 0 (disabled), 86400000 (1 day), 604800000 (7 days), 2592000000 (30 days)
			threadId (str | int): The thread ID (group ID or user ID)
			isGroup (bool): True if target is a group conversation, False if it is a user conversation

		Returns:
			dict: Decoded success response from Zalo
		...
		response = self._post(
			"https://tt-convers-wpa.chat.zalo.me/api/conv/autodelete/updateConvers",
			params=params,
			data=payload
		)
		data = response.json()
		...
		error_code = data.get("error_code")
		error_message = data.get("error_message") or data.get("data")
		raise ZaloAPIException(f"Error #{error_code} when updating auto delete chat: {error_message}")

	def checkGroup(self, url):
		"""Get group info by link.

		Args:
			url (str): Zalo group link (https://zalo.me/g/xxxxxx)

		Returns:
			dict: Decoded group info from Zalo
		"""
		params = {
			"zpw_ver": 685,
			"zpw_type": 30
		...
		error_code = data.get("error_code")
		error_message = data.get("error_message") or data.get("data")
		raise ZaloAPIException(f"Error #{error_code} when checking group: {error_message}")
	...
	def joinGroup(self, url):
		...
		Returns:
			dict: Decoded join status from Zalo
		"""
		params = {
			"zpw_ver": 685,
			"zpw_type": 30
		...
		error_code = data.get("error_code")
		error_message = data.get("error_message") or data.get("data")
		raise ZaloAPIException(f"Error #{error_code} when joining group: {error_message}")

	def leaveGroup(self, groupId, silent=False):
		"""Leave group.

		Args:
			groupId (str | list): The group ID or list of group IDs to leave
			silent (bool): Leave silently without system message. Default is False.

		Returns:
			dict: Decoded leave status from Zalo
		"""
		params = {
			"zpw_ver": 685,
			"zpw_type": 30
		}
		grids = [str(groupId)] if not isinstance(groupId, list) else [str(g) for g in groupId]
		payload = {
			"params": self._encode({
				"grids": grids,
				"imei": self.imei or self._state._config.get("imei"),
				"silent": 1 if silent else 0,
				"language": "vi"
			})
		}
		response = self._post(
			"https://tt-group-wpa.chat.zalo.me/api/group/leave",
			params=params,
			data=payload
		)
		data = response.json()
